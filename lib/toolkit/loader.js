...
const Require = iced(function Require(loader, requirer) {
  ...
  function require(id) {
    ...
    dump("require('toolkit/loader').require('" + id + "'');");

    let { uri, requirement } = getRequirements(id);
    let module = null;
    // If module is already cached by loader then just use it.
    if (uri in modules) {
      dump("require('toolkit/loader').require('" + id + "''); -> cached");
      module = modules[uri];
    }
    else if (isJSMURI(uri)) {
      dump("require('toolkit/loader').require('" + id + "''); -> JSM");
      module = modules[uri] = Module(requirement, uri);
      module.exports = Cu.import(uri, {});
      freeze(module);
    }
    else if (isJSONURI(uri)) {
      dump("require('toolkit/loader').require('" + id + "''); -> JSON");
      let data;
    ...
    if (!(uri in modules)) {
      ...
      try {
        ...
        dump("require('toolkit/loader').require('" + id + "''); -> " + uri);
      }
      catch (e) {
        dump(e);
    ...
    dump("require('toolkit/loader').require('" + id + "''); DONE!");
